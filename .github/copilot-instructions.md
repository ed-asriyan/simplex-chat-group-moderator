...
### Ask an AI (`ai.js`)
SimpleX carries text, and so does this: the editor writes a prompt, the owner pastes it into whatever AI chat they already use, and pastes the reply back. There is no API key, no request and no provider — the page stays static. The entry point is a button under the instructions, above the rules list, opening a `<dialog>` with the two steps — the same modal treatment `Apply changes` gets.
...
String lists longer than `BIG_LIST` (20, the same threshold that gives a list its filter and bulk mode) are replaced by a marker — `<<KEEP LIST 1: 347 ENTRIES>>` — and put back when the reply is pasted. This keeps a 347-word blocklist from being rewritten by a model that would quietly drop half of it, keeps the prompt small, and keeps a list of slurs away from a provider that would refuse it. The marker carries an **id, not a path**, because the model may reorder or duplicate rules; it stands in for the entries *in place*, so `["<<KEEP LIST 1: 347 ENTRIES>>", "new word"]` appends. The entry count in the marker is redundant on purpose: a mismatch is the only sign that the model edited a list it could not see, and it is refused rather than guessed at. Owners who need the entries themselves tick that list in the panel and the prompt carries it in full.
