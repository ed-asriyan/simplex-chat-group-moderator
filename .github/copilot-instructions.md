...
### Editing is undoable, not confirmed
The editor asks nothing before a destructive edit and instead keeps the last `UNDO_LIMIT` (20) states of `rules` — they are plain JSON, so a snapshot is a clone. Every change reaches the rules through one of three DOM listeners, and each snapshots before and calls `recordUndo` after, so no path can bypass the stack and a change that changes nothing records nothing. Undo and redo are the same move in opposite directions between two stacks (`history` / `future`); editing after an undo forks the timeline and discards `future`. Both buttons are **hidden**, not disabled, when their stack is empty — a control that can do nothing is noise, and the phone's action bar has room for three. Shortcuts: Ctrl/Cmd+Z, Ctrl/Cmd+Shift+Z and Ctrl+Y, all ignored inside a text field where the browser's own undo belongs.

There is one stack, reachable from two places: the header, and the AI panel's *Undo the load*, which sits where the owner already is when a pasted answer turns out wrong. That button compares the top of the stack with the snapshot the load pushed, so it can never quietly undo some later edit instead.

Two edits used to lose data with no way back, and both are now structural rather than guarded by a prompt: changing a condition's or action's type carries over every field the new type also has under the same name and kind (`retype`), which is what keeps a whole subtree when `All` becomes `Any` and the domains when one link condition becomes another; and a container can be **unwrapped** — its children splice into the parent when it sits inside another container, so nothing is dropped.

### Apply is the review point
`Apply changes` does not copy the link straight away. It opens a dialog with a diff of everything that changed since the page loaded — the baseline is simply the rules decoded from the hash, which is what the bot has stored — and copies only after the owner confirms. Rules have no ids, so the diff pairs identical rules first, uses a longest-increasing-subsequence over those to tell "moved" from "everything shifted", and matches the leftovers by resemblance (same condition type or same actions) so a delete plus an unrelated insert does not read as an edit. The same pairing runs one level down over a composite's children — identical ones first, the rest by condition type, whatever is left reported as removed (in red) or added, each one line naming the condition with its settings behind the same click a changed list takes, so a dropped rule carrying 300 words stays one line — because comparing them by position turns one deleted child into a run of bogus "X → Y" lines down the rest of the list; order inside `All`/`Any` means nothing to the bot, so a pure reorder is silent. A list that *shrank* is called out in red: it is the one change that would otherwise pass unnoticed. A successful copy moves the baseline to what was just sent. The dialog also warns when the copied link approaches `options.message_max_bytes`: past it the owner's client truncates the message, the bot reads a broken hash, and the failure surfaces in the chat with nothing pointing at the cause.

### Edit with AI (`ai.js`)
SimpleX carries text, and so does this: the editor writes a prompt, the owner pastes it into whatever AI chat they already use, and pastes the reply back. There is no API key, no request and no provider — the page stays static. The entry point is an `Edit with AI` button under the instructions, above the rules list, opening a `<dialog>` with the two steps — the same modal treatment `Apply changes` gets. Step 1 is one paragraph and `Copy the prompt`, in that order and with nothing between them: the owner who reaches for this button is the one who understands least, so the only thing to read is what the button does.

`ai.js` names no condition or action type either. The catalogue in the prompt is generated from `rules-schema.json` (type name, title, fields with their types and limits, and the full descriptions of both the type and its fields — that is where a model learns that matching is by whole word, or which message counts toward a rate limit), the container types and the phrasing about them are derived from which fields are `$ref`s, and the limits come from the `options` keys above. **A new rule type appears in the prompt on its own.**

String lists longer than `AI_BIG_LIST` (100 — deliberately not the editor's `BIG_LIST` of 20: twenty entries is where a list needs a filter in the UI, while a model reproduces a hundred words without dropping any) are replaced by a marker — `<<KEEP LIST 1: 347 ENTRIES>>` — and put back when the reply is pasted. This keeps a 347-word blocklist from being rewritten by a model that would quietly drop half of it, keeps the prompt small, and keeps a list of slurs away from a provider that would refuse it. The marker carries an **id, not a path**, because the model may reorder or duplicate rules; it stands in for the entries *in place*, so `["<<KEEP LIST 1: 347 ENTRIES>>", "new word"]` appends. The entry count in the marker is redundant on purpose: a mismatch is the only sign that the model edited a list it could not see, and it is refused rather than guessed at. Owners who need the entries themselves tick that list in the panel and the prompt carries it in full — behind a collapsed `Long lists` summary, absent entirely when no list is abbreviated, because a checkbox asking about placeholders is noise to everyone else.

Coverage is the one thing about actions a model cannot infer from the catalogue, and several have got it wrong the same way — advising an owner to add `ModerateMessage` beside a kick that already deletes the author's messages, which `action_planner` then drops. The prompt therefore carries a generated line per `covered_by` relation, and the editor's `covered` badge honours the same condition. When `covers` in `action_planner` changes, `covered_by` in the schema changes with it.

A pasted reply is parsed liberally (last ```json fence, else the first balanced array) and validated strictly against the schema; nothing loads unless it is clean, and what does load is a draft with an Undo — the write path is still `Apply changes`.
