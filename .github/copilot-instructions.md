...
1. **Domain:** add a variant to `ModerationCondition` (`message_filter/moderation_condition.rs`), implement its matching in `should_moderate_by_condition`, and give it an arm in `normalize_and_validate` so its parameters are checked when an owner saves the rule (add a submodule under `moderation_condition/` if the matching logic is non-trivial; keep it pure and unit-tested). `should_moderate` evaluates each rule's condition in order and short-circuits on the first match, returning a `ModerationMatch { action, reason }`.
